...
| — | **SECURITY OUTFLOW (2026-09-03)**: `detect_toolchain.resolve_tool` project-venv fallback (executes repo-committed `.venv/bin/<tool>` under the gate's authority) + `loop_state_path` project-dir fallback | **DIVERGED here** — pi port is PATH-only (opt-in via `SF_PROJECT_VENV_TOOLS=1`); loop_state fails closed. Upstream CC reference still carries both — candidate fix report to the CC repo |
