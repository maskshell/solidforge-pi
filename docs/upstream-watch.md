...
| #69 (ADR) | csr execution-process observability: csr_progress `stream` + all-runs view (session-scoped pointers); wrapper-distilled `round<k>-<provider>.stream.jsonl`; doc-findings `execution_trace`; agent Narration-discipline sections (4c1fcca, +1596) | ⚠️ pending — substrate-triaged candidate: CLI/schema/agent-contract parts absorbable; the narration-loop wiring stays un-adopted (ledger #62 row; pi's sf-subagents live panels already deliver the foreground-stream semantics); test additions need the #68-style CC-assertion strip |
