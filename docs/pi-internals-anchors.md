...
>
> **Fork delta (2026-09-03)**: `pi.namespace` (A3) is NOT in official pi 0.84.4 — it ships in the [maskshell/pi](https://github.com/maskshell/pi) fork (npm form `0.84.4+namespace.2` = release tag/asset form `v0.84.4-namespace.2`, the CI `NS_PIN`; proposal [earendil-works/pi#8834](https://github.com/earendil-works/pi/issues/8834)). A3 is therefore a FORK anchor: composed names exist only on fork builds, and the `namespace-e2e` CI job (which installs the fork release) is both the composition proof and the #8834 landing detector — when it lands, repoint that job at official pi and A3 moves to the official contract.
