...
Mirrors cross-source-review's plugin_layout.py (rule 7 -- copy the helper, do
NOT import a shared lib), adapted to primary-source-verification. psv ships its
OWN plugin.json (name/version/description/the skill entry). Phase A has NO hooks
of its own -- the pipeline is orchestrator-driven, NOT hook-driven -- so no
hooks.json; the hooks check SKIPS GRACEFULLY (rule 3) when absent.

Checks:
  - plugin.json parses + has name=primary-source-verification, version,
    description, author as an object, and a `skills` entry listing this skill.
  - agents/*.agent.md frontmatter (skill-internal, OPTIONAL -- psv's
    claim-extractor / claim-verifier are plugin-level at <repo-root>/agents/).
...
EXPECTED_PLUGIN_NAME = "primary-source-verification"
...
def _check_agents(findings, coverage):
    ...
    if not os.path.isdir(AGENTS_DIR):
        coverage.append(
            "no skill-internal agents/ -- psv's same-family legs (claim-extractor, "
            "claim-verifier) are plugin-level at <repo-root>/agents/ "
            "(solidforge:claim-extractor / solidforge:claim-verifier), mirroring "
            "csr's solidforge:doc-reviewer"
        )
        return
