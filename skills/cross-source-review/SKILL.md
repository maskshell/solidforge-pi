...
2. **Round loop** (same-family ↔ different-family, orchestrator-driven) — each round:
   ...
   - **Web-grounded claim verification (the web-claim-verifier spawn condition)** — at each reconcile moment, when a finding whose EVIDENCE — or whose `rejected` rationale — cites an external, FETCHABLE claim (fetchable = the claim names or cites an identifiable source — a URL, a standard/doc, a versioned product behavior — that a web fetch can retrieve; purely interpretive findings are not fetchable and take no spawn), and the finding's disposition is headed to `fixed` or `rejected`, invoke the `subagent` tool with `agent="solidforge:web-claim-verifier"` (fresh context, read-only; its contract is its own — do NOT open `agents/web-claim-verifier.agent.md`) BEFORE recording that disposition — bounded per reconcile moment (two moments per round — a round's composed ceiling is 4): at most 2 spawns per moment, highest-severity findings first; un-spawned claims carry the finding's own evidence and surface in the convergence-record's coverage notes (never silently skipped). `escalated` findings are already human-bound — no fetch spent (a fetch cannot change their destination). The spawn prompt: the finding's claim text + its defect_id (passed as the verifier's caller-supplied claim_id; state `claim` mode explicitly) + the cited URL or named source (+ optional version/scope context) — on the pi substrate the caller-supplied URL is the reliable fetch path (no native web tools). The returned verdict block's quote/URL/date drops into `<run-dir>/evidence/` and the disposition rationale cites it. Its optional `execution_trace` (ADR #69) persists the same way via trace-append to `<run-dir>/verify-<claim_id>.stream.jsonl` — absent trace tolerated.
   - Run the **different-family leg** — from the **project root** (the wrapper reads `<cwd>/.env.solidforge`), invoke the `hetero_doc_review` TOOL with `artifact: <doc>`, `authority: <ref-or-empty>`, `priorFindings: <round-json>` (fed the same-family findings as prior, so it hunts the gap, not restatements), and `progressFile: "<run-dir>/progress.jsonl"`, plus `roundIndex: <k>` (the stream log's filename and the sidecar's round labels derive from it — pass it; the wrapper falls back to prior-findings' round field, then 1, and a missed round makes legs append into the WRONG `round<k>-<provider>.stream.jsonl`) → doc-findings. **Do NOT pass `profile`** — the wrapper resolves the provider(s) from `HETERO_DOC_PROFILE` (loaded from `<cwd>/.env.solidforge`, shell wins; comma-list = dual-different-family; default `deepseek`); a hardcoded profile silently drops every other configured provider. Run from the project root — do NOT `cd` into the skill dir — or the token will not resolve. While it runs you have THREE live views, all zero-interaction: (1) the tool's own live per-provider panel — `leg-progress` events at event granularity (every grandchild tool call + turn: `read docs/x.md`, `turn 3 · $0.0123`) plus an ADR #52 `hetero-heartbeat` every 30s (elapsed / stream bytes / assistant events / the RESOLVED model / idle), derived from the wrapper's stderr and DISPLAY-ONLY — the tool result is the wrapper's stdout result JSON VERBATIM (`provider_runs[]` stamped in it), with progress lines never entering it; (2) the run-progress sidecar + the sf-progress footer strip (below); (3) `csr_progress.py status <run-dir> --watch 5` from any second terminal. FALLBACK (harnesses without the sf-hetero extension — plain `pi -p` without `-e`, non-pi shells): run `python3 "<skill-dir>/infra/scripts/hetero_doc_review.py" --artifact <doc> --authority <ref-or-empty> --prior-findings <round-json> --progress-file "<run-dir>/progress.jsonl" --round-index <k>` via bash — there the tool RESULT merges stderr with stdout, so treat leading `leg-progress` / `hetero-heartbeat` lines as progress, not errors, and parse the LAST JSON object on stdout; a pre-leg fail-fast (missing token / unknown profile) surfaces as a non-zero exit with the cause in that same output (via the tool: a clean `isError` result).
   - Reconcile again — the web-claim-verifier spawn condition above applies at THIS reconcile moment too (same trigger, same ≤2-per-moment cap). Apply the per-round reconciliation table (both-report→adopt; same-family-only→adopt; different-family-only→escalate; neither→pass; DEGRADED→adopt same-family).
...
A csr run is observable end to end at THREE layers, all zero-interaction (the pi-port replacement for upstream's ADR #62 narration loop — pi has no background bash, and substrate-level streaming needs no orchestrator polling): (1) IN-SESSION, PER-LEG — pi's bash tool streams the wrapper's stderr live (event-granularity `leg-progress` lines + the 30s heartbeat), and the `subagent` tool streams the same-family child's internals itself; (2) IN-SESSION, RUN-LEVEL — the `sf-progress` extension (bundled) tails the newest `progress.jsonl` and renders a condensed footer strip (`ctx.ui.setStatus`) — round k/cap, phase, findings, terminal state — visible no matter which tool is on screen; (3) EXTERNAL — any second terminal: `tail -f <run-dir>/progress.jsonl`, or `csr_progress.py status <run-dir> [--watch 5]` for a one-screen render (round k of cap, phase + last-event age, leg + reconcile totals, terminal state; torn last lines counted, never fatal). The `run-start` append also atomically points `runs/LATEST` at the new run dir (relative symlink, best-effort) — a human gets a stable one-liner, `csr_progress.py status workspace/cross-source-review/runs/LATEST --watch 5`, without knowing the timestamped dir name; in-session, the sf-progress footer strip remains the zero-interaction view). Distilled execution streams (ADR #69): every leg leaves `round<k>-<leg>.stream.jsonl` in the run dir — the wrapper distills the different-family leg live; the orchestrator persists the same-family leg's `execution_trace` at same-family-complete (step 2); render readably with `csr_progress.py stream <run-dir> [--tail N | --watch 5]`. `runs/LATEST` is single-active-run-only (concurrent sessions steal it — state the run-dir path at Frame instead); for concurrent sessions, `csr_progress.py status workspace/cross-source-review/runs/` renders one block per ACTIVE run (all-runs view, ADR #69).
