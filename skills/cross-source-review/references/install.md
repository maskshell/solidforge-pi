...
- **In-session, per-leg (live)** — the `hetero_doc_review` tool (sf-hetero extension) renders a live per-provider panel from the wrapper's stderr: `leg-progress` lines at event granularity (every reviewer tool call — `read docs/x.md` — and every turn — `turn 3 · $0.0123`) plus an ADR #52 `hetero-heartbeat` every 30s (elapsed / bytes / assistant events / the RESOLVED model / idle); the tool RESULT stays the wrapper's stdout JSON verbatim (progress never enters it). Without the extension (plain `pi -p`, non-pi shells) the bash fallback streams the same stderr lines live into the bash tool panel. The same-family leg streams its own internals via the `subagent` tool.
...
- **External** — any second terminal: `tail -f <run-dir>/progress.jsonl`, or `python3 <skill-dir>/infra/scripts/csr_progress.py status <run-dir> --watch 5` for a one-screen render. The sidecar is written by the orchestrator (run/round boundaries via `csr_progress.py append`) and by the wrapper (`--progress-file <run-dir>/progress.jsonl` — pass it on every hetero invocation; best-effort, never fails the review). Observability beyond the sidecar (ADR #69): every leg leaves a distilled `round<k>-<leg>.stream.jsonl` in the run dir (the wrapper distills the different-family leg live; the orchestrator persists the same-family leg's execution_trace) — render with `csr_progress.py stream <run-dir> [--tail N | --watch 5]`; concurrent sessions watch the all-runs view (`csr_progress.py status workspace/cross-source-review/runs/`) and state their run-dir path in-conversation at Frame (runs/LATEST is single-active-run-only).
